...
metadata_path = arguments[3]
#...
book = open_workbook(input_path)		# Change the path according to the file location
...
#...
book1 = open_workbook(metadata_path)		# Change the path according to the file location
...
print(len(rid1))
owners = set(rid1)
print('the number of reviews owners is : {} '.format(owners)) 
avg_rat = []
r_dev = []
rank = []	
tpid = []
...
def MNR_RPR_RNR_ARD_WRD_RBST_FRR_EXRR_MRD_ERR_TRRR_BRRR():
	...
	while (i < len(rid)):
		...
		TRRR_21 = float(count_top20s)/len(temp_top20s)
		BRRR_21 = float(count_bottom20s)/len(temp_bottom20s)
		TRRR_20.append(float(TRRR_21))
		BRRR_20.append(float(BRRR_21))
		...
		i = i + count[i]	
print('features lengthes : ',len(rpr), len(rnr), len(mnr), len(ard), len(wrd), len(rbst),len(frr), len(exrr), len(mrd), len(err),len(TRRR_20), len(BRRR_20))
...
df.to_excel(output_path+"/Reviewer_Centric_Behavioral_Features.xlsx", index = False)		# Change the path according to the file location
