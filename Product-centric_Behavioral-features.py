...
print('length of rid is {} \n'.format(len(rid)))
print('length of product id is {} \n'.format(len(pid)))
pure_products = set(pid)
print('length of pure_products is : {} '.format(len(pure_products)))
avg_rat = []
r_dev = []
rank = []	
tpid = []
...
def Product_MNR_RPR_RNR_ARD_WRD_PBST_EXRR_MRD_ERR_TRRR_BRRR():
	...
	while (i < len(pid)):
		...
		TRRR_21 = float(len(temp_top20))/len(temp_date)
		BRRR_21 = float(len(temp_bottom20))/len(temp_date)
...
print('feature lengthtes : ',len(rpr), len(rnr), len(mnr), len(ard), len(wrd), len(pbst), len(exrr), len(mrd), len(err),len(TRRR_20), len(BRRR_20))
...
print(df)
df.to_excel(output_path+"/Resulting Features_Product_Centric_Behavioral_Features.xlsx", index = False)		# Change the path according to the file location
