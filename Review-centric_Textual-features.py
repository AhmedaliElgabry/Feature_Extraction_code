...
positive_path = arguments[3]
negative_path = arguments[4]
...
def RFPP_RSPP_RFTAPP_RSAPP_RSW_ROW_RInW_RImW_RPW_RNW():
	...
	positive_f = io.open(positive_path, 'r',encoding  =  'utf-8', errors = 'ignore')		# Change the path according to the file location
	positive_lines = positive_f.readlines()
	negative_f = io.open(negative_path, 'r', encoding  = 'utf-8', errors = 'ignore')		# Change the path according to the file location
...
df.to_excel(output_path+"/Resulting Features\\Review_Centric_Textual_Features.xlsx", index = False)			# Change the path according to the file location
